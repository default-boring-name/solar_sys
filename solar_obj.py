...
class Objects:
    def __init__(self, x, y, v_x, v_y, color, r, m):
        '''
        инициализаци€ класса объектов солнечной системы
        x - координата x
        y - координата y
        v_x - горизонтальна€ скорость 
        v_y - вертикальна€ скорость
        color - цвет
        r - радиус
        m - масса
        '''
    ...
    def calculate_force(self, objects):
        ...
        for obj in objects:
            if obj != self:
                ...
                #проверка слишком близкого сближени€
                if l < (self.r + obj.r):
    ...
    def move(self, dt):
        '''
        передвижение тела за определенное врем€
        dt - врем€, за которое рассматриваетс€ изменение
        '''
